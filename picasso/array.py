...
Defines a shallow wrapper around numpy.ndarray for extra functionality.

For most purposes, the differences between numpy.ndarray and
array.SurveyArray are not important. 

"""

#To Do: remove the units parts from this, or leave it, but don't use it....
...
class SurveyArray(np.ndarray):
    ...
    @property
    def ancestor(self):
        """Provides the basemost SurveyArray that an IndexedSurveyArray is based on."""
        return self
    ...
    def __pow__(self, x):
        ...
        r = np.asarray(np.power(self.view(np.ndarray), numerical_x)).view(SurveyArray)
...
for f in np.ndarray.__lt__, np.ndarray.__le__, np.ndarray.__eq__, \
    ...
    setattr(SurveyArray, f.__name__, wrapper_function)
...
class IndexedSurveyArray(object):
    ...
    def __reduce__(self):
        return SurveyArray(self).__reduce__()
...
# The IndexedSurveyArray class is now supplemented by wrapping all the
...
def _wrap_fn(w):
    def q(s, *y,  **kw):
        ...
        if kw != {}:
            return w(SurveyArray(s), *y, **kw)
        else:
            return w(SurveyArray(s), *y)
...
for x in set(np.ndarray.__dict__).union(SurveyArray.__dict__):
    w = getattr(SurveyArray, x)
    if 'array' not in x and ((not hasattr(IndexedSurveyArray, x)) or x in _override) and hasattr(w, '__call__'):
        setattr(IndexedSurveyArray, x, _wrap_fn(w))
...
def _array_factory(dims, dtype, zeros, shared):
    ...
    if shared and posix_ipc:
        ...
        if zeros:
            ret_ar = np.zeros(dims, dtype=dtype).view(SurveyArray)
        else:
            ret_ar = np.empty(dims, dtype=dtype).view(SurveyArray)
    return ret_ar
...
if posix_ipc:
    ...
    def _shared_array_deconstruct(ar, transfer_ownership=False):
        ...
        assert isinstance(ar, SurveyArray)
        ar_base = ar
        while isinstance(ar_base.base, SurveyArray):
    ...
    def _shared_array_reconstruct(X):
        ...
        mapfile = mmap.mmap(mem.fd, mem.size)
        size = reduce(np.multiply, dims)
        # new_ar =  np.memmap(mem.fd, dtype=dtype, shape=dims, mode='r+').view(SurveyArray)
        new_ar = np.frombuffer(
            mapfile, dtype=dtype, count=size, offset=offset).reshape(dims).view(SurveyArray)
    ...
    def _recursive_shared_array_deconstruct(input, transfer_ownership=False) :
        ...
        output = []
        for item in input:
            if isinstance(item, SurveyArray):
