...
families of data products (e.g. truth, prediction, or different resolutions).
New Family objects are automatically registered so that
surveys can use them in the normal syntax (survey.truth,
survey.prediction, etc).

In practice the easiest way to make use of the flexibility
this module provides is through adding more families of
data in your config.ini.
...
def family_names(with_aliases=False):
    """Returns a list of the names of all data families.
