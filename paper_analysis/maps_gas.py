...
true_label = [r'$M_{\rm gas}^{\rm true}$',r'$Z_{\rm gas}^{\rm true}$',r'SFR$^{\rm true}$']
pred_label = [r'$M_{\rm gas}^{\rm pred}$',r'$Z_{\rm gas}^{\rm pred}$',r'SFR$^{\rm pred}$']
diff_label = [r'$\Delta M_{\rm gas}$',r'$\Delta Z_{\rm gas}$',r'$\Delta$SFR$_{\rm star}$'] 

star_props = ['gas_Masses','gas_GFM_Metallicity','gas_StarFormationRate']
cmaps = ['cividis','inferno','YlOrRd'] #'Spectral_r']
cmap_label = [r'$\log(M_{\rm gas}/M_{\odot})$',r'$\log(M_Z^{\rm gas}/M_{\odot})$',r'$\log$(SFR/[$M_\odot\rm{yr}^{-1}$])']
delta_label = [r'$\log(M_{\rm gas}^{\rm true}/M_{\rm gas}^{\rm pred})$',r'$\log(Z_{\rm gas}^{\rm true}/Z_{\rm gas}^{\rm pred})$',r'$\log($SFR$^{\rm true}/$SFR$^{\rm pred})$']
...
for galaxy in survey['galaxy']: 
    ...
    for i, ax in enumerate(true_phys_axes):
        ...
        if star_props[i] == 'stars_GFM_StellarFormationTime':
            ...
            try:
                min_val =  np.nanmin(true_map[np.isfinite(true_map)])
            except:
                min_val = 4.
            try:
                max_val =  np.nanmax(true_map[np.isfinite(true_map)])
            except:
                max_val = 8.
    ...
    plt.savefig('./maps_gas/'+str(galaxy.galaxy_id)+'_masked_gas.pdf', bbox_inches='tight')
